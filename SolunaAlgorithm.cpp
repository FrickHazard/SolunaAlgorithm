            ...
            for (uint32_t j = 0; j < result.size(); ++j) {
                for (uint32_t k = 0; k < combinations.size(); ++k) {
                    ...
                    for (uint32_t l = 0; l < combinations[k].size(); ++l) {                     
                            newGameState.pieceLists[l].push_back({ (uint16_t)currentHeight, mergingColorPartition[a][l] });             
                    }
                    // recount colors
                    for (uint32_t l = 0; l < COLOR_COUNT; ++l) {
                        if (newGameState.pieceLists[l].size() == 0) {
...
void getAllSymmertricBoardSpaces()
{
    ...
    for (uint32_t i = 0; i < allGameStates.size(); ++i) {       
        std::vector<MoveResult> moves = getPossibleMoves(allGameStates[i]);
    }
}
